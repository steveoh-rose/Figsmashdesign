...
function buildUrlPayload() {
	...
	var col = collectNodes(frame);
	if (col.error || !col.nodes.length) {
		urlError = col.error || 'no nodes';
		return null;
	}
	var out = col.nodes.map(function (item) {
		return {
			name: item.node.name,
			x: Math.round(item.bb.x - col.fbb.x),
			y: Math.round(item.bb.y - col.fbb.y),
			w: Math.round(item.bb.width),
			h: Math.round(item.bb.height),
			fill: solidFill(item.node),
		};
	});
	try {
